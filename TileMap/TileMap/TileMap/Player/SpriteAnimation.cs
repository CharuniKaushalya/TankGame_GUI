...
namespace TileMap.Player
{
    class SpriteAnimation
    {
        ...
        public void getRotation(int x)
        {
            if (x == 0) { fRotation = (float)-1.570796 ; }
            if (x == 1) { fRotation = 0; }
            if (x == 2) { fRotation = (float)1.570796; }
            if (x == 3) { fRotation = (float)3.141593; }
        ...
        void UpdateRotation()
        {
            if (bRotateByPosition)
            {
               ...
               // Console.WriteLine("frotation : " +fRotation);
            }
        }
